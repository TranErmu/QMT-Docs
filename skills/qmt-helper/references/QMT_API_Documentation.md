...
- [快速开始](#快速开始)
- [XtQuant.XtData 行情模块](#XtQuantXtData-行情模块)
- [XtQuant.Xttrade 交易模块](#XtQuantXttrade-交易模块)
- [完整实例](#完整实例)
- [常见问题](#常见问题)
- [xtquant版本下载](#xtquant版本下载)
...
- 返回 
  - dict，可转债信息，具体字段参考 [可转债信息字典](/dictionary/bond.html#%E8%8E%B7%E5%8F%96%E5%8F%AF%E8%BD%AC%E5%80%BA%E4%BF%A1%E6%81%AF)
...
| 属性 | 类型 | 注释 |
| --- | --- | --- |
| account_type | int | 账号类型，参见[数据字典](#-数据字典) |
| account_id | str | 资金账号 |
| cash | float | 可用金额 |
...
| order_time | int | 报单时间 |
| order_type | int | 委托类型，参见[数据字典](#-数据字典) |
| order_volume | int | 委托数量 |
| price_type | int | 报价类型，该字段在返回时为柜台返回类型，不等价于下单传入的price_type，枚举值不一样功能一样，参见[数据字典](#-数据字典) |
| price | float | 委托价格 |
| traded_volume | int | 成交数量 |
| traded_price | float | 成交均价 |
| order_status | int | 委托状态，参见[数据字典](#-数据字典) |
| status_msg | str | 委托状态描述，如废单原因 |
| strategy_name | str | 策略名称 |
| order_remark | str | 委托备注，最大 24 个英文字符 |
| direction | int | 多空方向，股票不适用；参见[数据字典](#-数据字典) |
| offset_flag | int | 交易操作，用此字段区分股票买卖，期货开、平仓，期权买卖等；参见[数据字典](#-数据字典) |
...
| order_sysid | str | 柜台合同编号 |
| strategy_name | str | 策略名称 |
| order_remark | str | 委托备注，最大 24 个英文字符(一个中文占3个) |
| direction | int | 多空方向，股票不适用；参见[数据字典](#-数据字典) |
| offset_flag | int | 交易操作，用此字段区分股票买卖，期货开、平仓，期权买卖等；参见[数据字典](#-数据字典) |
...
| direction | int | 多空方向，股票不适用；参见[数据字典](#-数据字典) |
| hedge_flag | int | 投保类型；参见[投保类型](#-投保类型) |
| position | int | 持仓数量 |
...
| login_status | int | 账号状态，参见[数据字典](#-数据字典) |
...
- 返回 
  - 该账号对应的资产对象 [XtAsset](#-资产XtAsset)或者None 
...
- 返回 
  - 该账号对应的当日所有委托对象 [XtOrder](#-委托XtOrder)组成的list或者None 
...
- 返回 
  - 该账号对应的当日所有成交对象 [XtTrade](#-成交XtTrade)组成的list或者None 
...
- 返回 
  - 该账号对应的最新持仓对象 [XtPosition](#-持仓XtPosition)组成的list或者None 
...
- 返回 
  - 该账号对应的最新持仓对象 [XtPositionStatistics](#-期货持仓统计XtPositionStatistics)组成的list或者None 
...
- 返回 
  - 该信用账户对应的资产对象 [XtCreditDetail](#-信用账号资产XtCreditDetail)组成的list或者None 
...
- 返回 
  - 该账户对应的负债合约对象 [StkCompacts](#-负债合约StkCompacts)组成的list或者None 
...
- 返回 
  - 该账户对应的融资融券标的对象 [CreditSubjects](#-融资融券标的CreditSubjects)组成的list或者None 
...
- 返回 
  - 该账户对应的可融券数据对象 [CreditSloCode](#-可融券数据CreditSloCode)组成的list或者None 
...
- 该账户对应的标的担保品对象 [CreditAssure](#-标的担保品CreditAssure)组成的list或者None 
...
- 参数 
  - data - [XtAccountStatus](#-账号状态XtAccountStatus)账号状态信息 
...
- 参数 
  - data - [XtOrderError](#-下单失败错误XtOrderError)下单失败信息 
...
- 参数 
  - data - [XtCancelError](#-撤单失败错误XtCancelError)撤单失败信息 
...
- 参数 
  - data - [XtOrderResponse](#-异步下单委托反馈XtOrderResponse)异步下单委托反馈 
...
- 参数 
  - data - [XtSmtAppointmentResponse](#-约券相关异步接口的反馈XtSmtAppointmentResponse)约券相关异步接口的反馈 
...
1. 如果miniqmt开启后, userdata_mini文件夹内没有 `up_queue_xtquant`文件，说明没有用户没有对应函数下单的权限，需要联系券商开启 
...
[指定session id范围连接交易](#-指定session-id范围连接交易)
